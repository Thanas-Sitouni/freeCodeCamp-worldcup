...
echo "$($PSQL "SELECT SUM(goals) FROM (SELECT winner_goals as goals FROM games UNION ALL SELECT opponent_goals FROM games) tg")"
...
echo "$($PSQL "select year, name from teams t inner join (SELECT year, winner_id FROM games WHERE round = 'Final') yt on t.team_id=yt.winner_id order by year asc")"
